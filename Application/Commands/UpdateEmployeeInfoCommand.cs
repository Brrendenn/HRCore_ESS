...
public class UpdateEmployeeInfoCommand(int employeeId, UpdateEmploymentInfoDto commandDto) : IRequest<string>
{
    ...
    public class Handler(IEmployeeRepository employeeRepository) : IRequestHandler<UpdateEmployeeInfoCommand, string>
    {
        public async Task<string> Handle(UpdateEmployeeInfoCommand command, CancellationToken cancellationToken)
        {
            ...
            employee.EmploymentInformation.EmploymentStatus = command.EmploymentStatus;
            employee.EmploymentInformation.StartDate = command.StartDate;
            employee.EmploymentInformation.EmploymentType = command.EmploymentType;
            employee.EmploymentInformation.Department = command.Department;
            employee.EmploymentInformation.Position = command.Position;
            employee.EmploymentInformation.SupervisorName = command.SupervisorName;
